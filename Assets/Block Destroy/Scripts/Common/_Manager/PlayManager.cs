...
public class PlayManager : Singleton<PlayManager>
{
    ...
    private void Awake()
    {
        ...
        // Subscribe to scene load request events
        EventBus.Instance.Subscribe<SceneLoadRequestEvent>(OnSceneLoadRequest);
    }
    ...
    private void OnDestroy()
    {
        ...
        {
            EventBus.Instance.Unsubscribe<SceneLoadRequestEvent>(OnSceneLoadRequest);
        }
    }

    // Event handler for scene load requests
    private void OnSceneLoadRequest(SceneLoadRequestEvent evt)
    {
        LoadScene(evt.SceneName, evt.IsRefresh);
    }
    ...
    IEnumerator LoadSceneCo(string sceneName, bool isRefresh = false)
    {
        ...
        EventBus.Instance.Publish(new SceneLoadedEvent(sceneName));
    }
    ...
    void SceneCheck()
    {
        ...
        else if (scenename == Data.scene_arcade)
        {
            sceneType = SceneType.Arcade;
        }
        else if (scenename == Data.scene_result_arcade)
        {
            sceneType = SceneType.ResultArcade;
        }
    }
    ...
    public void ReportScore(int score)
    {
        ...
        if (Social.localUser.authenticated == true) {
            ...
            Social.localUser.Authenticate((bool success) => {
                if (success) {
                    Social.ReportScore(score, GPGSIds.leaderboard_leaderboard, (bool successs) => { });
                }
            });
        }
#endif
...
#if UNITY_ANDROID
        if (Social.localUser.authenticated == true) {
            Social.ReportScore(turn, GPGSIds.leaderboard_longest_turns, (bool result) => { });
        } else {
            Social.localUser.Authenticate((bool success) => {
                if (success) {
                    Social.ReportScore(turn, GPGSIds.leaderboard_longest_turns, (bool result) => { });
