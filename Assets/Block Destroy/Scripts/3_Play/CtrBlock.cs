...
public class CtrBlock : MonoBehaviour
{
    ...
    //Delete all blocks except bonus balls after watching rewarded ad
    public void DestroyContinueBlock()
    {
        SoundManager.Instance.PlayEffect(SoundList.sound_play_sfx_revival);

        // Destroy all block groups except bonus balls
        int groupCount = blockGroups.Count;
        for (int i = groupCount - 1; i >= 0; i--)
        {
            if (i < blockGroups.Count)
            {
                blockGroups[i].DestoryAllExceptBalls();
            }
        }
    }
