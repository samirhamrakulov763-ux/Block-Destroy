...
public class CtrResultArcade : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        // Increment game count for ads
        if (ADManager.Instance != null)
        {
            ADManager.Instance.IncrementGameCount();
        }
    ...
    IEnumerator ShowResultCo()
    {
        ...
        if (playScore > bestScore)
        {
            ...
            //PlayerPrefs saves highscores for Arcade mode
            GameData.BestScoreArcade = score;
            GameData.BestTurnArcade = turn;

            // Отправляем счет на Firebase сервер (Arcade режим)
            if (FirebaseLeaderboardManager.Instance != null)
            {
                FirebaseLeaderboardManager.Instance.SubmitScore(
                    GameData.NickName,
                    score,
                    turn,
                    true, // Arcade режим
                    (success) => {
                        if (success)
                        {
                            Debug.Log("Arcade score submitted to Firebase successfully!");
                        }
                    }
                );
            }
    ...
    void RePlay()
    {
        ...
        PlayManager.Instance.LoadScene(Data.scene_arcade);
    }
...
// Закомментировано из-за конфликта с новым Input System
// Если нужно, можно переписать на новый Input System
/*
#if UNITY_ANDROID
    private void Update () {
